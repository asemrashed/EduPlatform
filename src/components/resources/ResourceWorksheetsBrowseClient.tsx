...
export function ResourceWorksheetsBrowseClient({
  ...
  const grouped = useMemo(() => {
    const map = new Map<string, ResourceWorksheetRow[]>();
      ...
      {showPageHeader ? (
        <header className="mb-6">
          ...
          <p className="mt-2 max-w-2xl text-sm text-muted-foreground">
            Practice worksheets linked to batch modules or course lessons.
          </p>
        </header>
      ) : null}
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        <Badge variant="outline">{row.scopeType === "batch" ? "Batch" : "Course"}</Badge>
                    ...
                    {row.canDownload ? (
                      <Button asChild size="sm">
                        <a
                          href={resourceWorksheetsService.downloadHref(row._id)}
                          target="_blank"
