  ...
  const renderEmptyState = () => {
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-500 text-lg">{htmlToPlainText(empty.description)}</p>
      </div>
    );
  };
      ...
      <table className="w-full min-w-[800px]">
        <thead className="bg-gradient-to-r from-gray-50 to-blue-50">
          <tr>
            ...
            {actions.length > 0 && (
              <th className={`px-4 sm:px-6 py-3 sm:py-4 text-center text-xs sm:text-sm font-semibold text-gray-700 uppercase tracking-wide sticky right-0 bg-gradient-to-l from-blue-50 to-gray-50 shadow-[-4px_0_8px_-2px_rgba(0,0,0,0.05)] ${actionColumnWidthClass}`}>
                Actions
              </th>
            )}
          </tr>
        ...
        <tbody className="divide-y divide-gray-200">
          {data.map((item, index) => (
            ...
            >
              ...
              {actions.length > 0 && (
                <td className={`px-4 sm:px-6 py-3 sm:py-4 text-sm text-gray-900 sticky right-0 bg-white group-hover:bg-gray-50 shadow-[-4px_0_8px_-2px_rgba(0,0,0,0.05)] ${actionColumnWidthClass}`}>
                        ...
                        <Button
                          ...
                          className={`h-8 w-8 ${
                            action.variant === 'destructive'
                              ...
                              : ''
                          } ${getActionClassName(action, item)}`}
                          onClick={(e) => {
                            e.stopPropagation();
                            handleActionClick(action, item);
                          }}
                          title={getActionLabel(action, item)}
                        >
                      ...
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-48 bg-white border border-gray-200 shadow-lg">
                          {actions.map((action) => (
                            ...
                            >
                              {getActionIcon(action, item) && <span className="mr-3">{getActionIcon(action, item)}</span>}
                ...
                <DropdownMenu>
                  ...
                  >
                    {actions.map((action) => (
                      ...
                      >
                        {getActionIcon(action, item) && <span className="flex-shrink-0 [&>svg]:w-5 [&>svg]:h-5 sm:[&>svg]:w-4 sm:[&>svg]:h-4">{getActionIcon(action, item)}</span>}
                        <span className="font-medium">{getActionLabel(action, item)}</span>
                      </DropdownMenuItem>
                    ))}
                  </DropdownMenuContent>
                </DropdownMenu>
