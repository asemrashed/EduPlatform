...
EnrollmentSchema.virtual("courseInfo", {
  ...
  foreignField: "_id",
  justOne: true,
  select: "title description thumbnailUrl price category isPaid duration",
});
...
EnrollmentSchema.pre("save", function () {
  ...
  if (this.isModified("certificateIssued")) {
    if (this.certificateIssued) {
      this.certificateIssuedAt = this.certificateIssuedAt || now;
    } else {
...
EnrollmentSchema.statics.getCourseEnrollmentCount = async function (
  courseId: string,
) {
...
EnrollmentSchema.statics.getStudentEnrollmentCount = async function (
  studentId: string,
) {
  return this.countDocuments({
    student: studentId,
